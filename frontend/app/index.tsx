...
const USDC_CONTRACTS = {
  ...
  VIRTUAL_POLYGON: '0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359', // ⭐ Virtual Polygon
...
const RPC_URLS = {
  ...
  VIRTUAL_POLYGON: 'https://virtual.polygon.eu.rpc.tenderly.co/06d8f56a-6ac8-4ef1-8e21-e79ea1fafe53', // ⭐ Virtual Polygon
...
const NETWORK_NAMES = {
  VIRTUAL_POLYGON: '⭐ Virtual Polygon (50k USDC)', // Default - User's preferred
  VIRTUAL_MAINNET: 'Virtual Mainnet (Ethereum)',
...
const EXPLORERS = {
  VIRTUAL_POLYGON: 'https://polygonscan.com',
...
export default function Index() {
  ...
  const [network, setNetwork] = useState('VIRTUAL_POLYGON'); // ⭐ Virtual Polygon is default (50k USDC)
  ...
  const loadBalances = async () => {
    ...
    try {
      let rpcUrl = RPC_URLS[network] || RPC_URLS.VIRTUAL_POLYGON;
      ...
      } else if (network === 'POLYGON' || network === 'VIRTUAL_POLYGON') {
        usdcContract = USDC_CONTRACTS.POLYGON; // Polygon USDC
      } else {
        usdcContract = USDC_CONTRACTS.POLYGON; // Default to Polygon
      }
  ...
  const switchNetwork = () => {
    const networks = ['VIRTUAL_POLYGON', 'VIRTUAL_MAINNET', 'POLYGON', 'BASE', 'ETH', 'VIRTUAL_BASE']; // Virtual Polygon first
